...
result_data = queryresult(repo, queryAjccSex)
hnscc = pd.read_csv(StringIO(result_data))
for col in hnscc.columns:
    hnscc[col] = hnscc[col].map(codedict).fillna(hnscc[col])
ajccDatasize = hnscc.groupby(["Gender", "AJCC", "TumourLocation"]).size()
y = ajccDatasize.to_dict()
x = {}
for key, val in y.items():
    x[key] = {}
    x[key]['Gender'] = key[0]
    x[key]['AJCC'] = key[1]
    x[key]['TumourLocation'] = key[2]
    x[key]['Count'] = val
mydict['AjccSex'] = list(x.values())
